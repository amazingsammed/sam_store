...
import {
    ...
    deleteCategory, deleteStockGroup,
    getStockGroup,
    getStockUnits
...
function StockGroupAction({element}) {
    const param = useParams();
    const router = useRouter();
    const path = usePathname();
    async function handleDeactivate() {
        await deactivateStockGroup(element,param.storeid);
        await router.refresh();
    }
    async function handleDelete() {
        await deleteStockGroup(element,param.storeid);
        await router.refresh();
    }
    return (
        <DropdownMenu>
...
export default StockGroupAction