...
import {z} from "zod";
import {createCashPurchases, createCreditPurchases} from "@/app/_actions/purchases";
...
import {SupplierListCombo} from "@/app/stores/_component/supplierlistcombo";


export default function CreditPurchasesFormPage(prop) {
    ...
    async function handleSave(e,data) {
       ...
       const [voucher ,accounting ,inventory] = await createCreditPurchases(list,supplier, path.storeid);
       if(voucher &&accounting&&inventory){
        ...
        toast.success('Credit purchases success');
       }else {
           toast.error('Something went wrong : credit purchase');
       }
    }
    ...
    return (
        <div>
            ...
            <div className='h-[80dvh]  justify-between col-span-10'>
                <HeaderWithButton title='Credit Purchases Form'
                                  subtitle='Used to record purchases made on Credit'
                                  bname="Save"
                                  ontap={action}>
                    <div>
                        <SupplierListCombo onChange={handleSupplierChange} onPressed={(e)=>console.log(e)}> </SupplierListCombo>
