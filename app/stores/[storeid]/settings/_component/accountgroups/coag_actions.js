...
import {deactivateCOAG, editCOAG} from "@/app/_actions/account";
import {accounts} from "@/app/stores/[storeid]/settings/_component/chartofaccount/coa_form";

function CoagAction({element}) {
    const param = useParams();
    const router = useRouter();
    const path = usePathname();

    async function handleDeactivateCoag() {
        await deactivateCOAG(element, param.storeid);
        router.refresh();
    }
...
export default CoagAction;


function EditItemForm(prop) {
    const [data, setData] = useState({
        'name': prop.prop.name,
        'accountid': prop.prop.accountid,
        'uuid': prop.prop.uuid,
    });
    ...
    async function handleEditStockitem(state, datax) {
        await editCOAG(datax, path.storeid);
        router.refresh();
    }
